...
void MDIO_voidSetPinDirection(u8 ARG_u8Port,u8 ARG_u8Pin,u8 ARG_u8Directoin)
{
	if(ARG_u8Pin<=7)
	{
		...
		else if(MDIO_INPUT_PULLUP==ARG_u8Directoin)
		{
			switch(ARG_u8Port)
			{
				case MDIO_PORTB: CLEAR_BIT(GPIOB -> DDR, ARG_u8Pin); SET_BIT(GPIOA -> DDR, ARG_u8Pin); break;
				case MDIO_PORTC: CLEAR_BIT(GPIOC -> DDR, ARG_u8Pin); SET_BIT(GPIOB -> DDR, ARG_u8Pin); break;
				case MDIO_PORTA: CLEAR_BIT(GPIOA -> DDR, ARG_u8Pin); SET_BIT(GPIOC -> DDR, ARG_u8Pin); break;
				case MDIO_PORTD: CLEAR_BIT(GPIOD -> DDR, ARG_u8Pin); SET_BIT(GPIOD -> DDR, ARG_u8Pin); break;
				default: break;/*report error*/
			}			
		}	
		else
...
void MDIO_voidSetPinValue(u8 ARG_u8Port,u8 ARG_u8Pin,u8 ARG_u8Value)
{
	if(ARG_u8Pin<=7)
	{
		...
		else if(MDIO_LOW==ARG_u8Directoin)
		{
			switch(ARG_u8Port)
			{
